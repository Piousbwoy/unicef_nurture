...
async function handleSync(req, res) {
  ...
  try {
    ...
    // Audit trail: what changed, never the PHI itself.
    await conn.execute(
      'INSERT INTO sync_log (entity_table, entity_id, operation, rows_affected, client_version) ' +
        'VALUES (?, ?, ?, ?, ?)',
      [entityTable, String(entityId ?? ''), operation, affected, clientVersion]
    );
