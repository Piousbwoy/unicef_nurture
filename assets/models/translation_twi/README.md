# Twi neural translation model (en → tw)

MarianMT encoder/decoder pair used by the offline caregiver voice pipeline.

- **Source weights:** `Helsinki-NLP/opus-mt-en-tw` — see `provenance.json` for the
...
57,000 × 512 matrix a second time. Splitting it into `proj_model.onnx` is what
keeps every file under GitHub's 100 MB limit.

`final_logits_bias` is not optional. Marian's output layer is
`logits = hidden @ Eᵀ + bias`, and the bias is trained and large; dropping it
produces repetition loops and truncated sentences.
...
`build/marian_reference/translation_twi/` token-for-token.
