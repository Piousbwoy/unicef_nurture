...
void main() {
  ...
  test('concurrent initialization deduplicates and failure can retry', () async {
    final runner = _Runner()..gate = Completer<void>()..fail = true;
    final service = PiperTtsService.debugCreate(runner: runner);
  ...
  test('readiness follows runner eviction rather than stale service state', () async {
    final runner = _Runner();
    final service = PiperTtsService.debugCreate(runner: runner);
    await service.initializeLanguage('Twi');
...
class _Runner implements PiperTtsRunner {
  ...
  @override bool hasModel(String language) => loaded == language;
  @override Future<void> init({required String language, required String modelAssetPath, required int speakerId}) async {
    loads++;
    await gate?.future;
    if (fail) throw StateError('Transient load failure');
    loaded = language;
  }
  @override Future<void> dispose(String language) async { if (loaded == language) loaded = null; }
  @override void setActiveLanguage(String language) {}
  @override Future<void> speak(String text, {bool waitForCompletion = true, void Function()? onStarted}) async {}
  @override Future<void> speakNonBlocking(String text) async {}
